...
from BACKUPS.networks import make_resnet, make_classifier
...
def make_model(
    ...
    first_feature_maps: int, large: bool, n_epochs: int, max_lr: float
) -> Model:
    ...
    classifier_num_params = sum(p.numel() for p in classifier.parameters() if p.requires_grad)
    print(f"Device : {device}")
    print(f"Num params : {backbone.num_params:,} (backbone) + {classifier_num_params} (classifier)")
    ...
    scheduler = torch.optim.lr_scheduler.OneCycleLR(optimizer, max_lr=max_lr, **length_params)
...
def main(
    *,
    ...
    rootdir: str, batch_size: int, max_lr: float, n_epochs: int, output_dir: str, name: str = None
) -> None:
    if name is None:
        name = f"{backbone}_{dataset}"
    data = make_data(dataset, rootdir, batch_size)
    model = make_model(data, dataset, backbone, first_features_maps, large, n_epochs, max_lr)
